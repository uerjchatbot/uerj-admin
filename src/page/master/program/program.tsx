...
//? Edit Modals
import { IMasterDefaultData, IMasterProgram, IMasterTeacher } from "@/models/master";
...
const Program = () => {
  ...
  const [nodeQuestion, setNodeQuestion] = useState<IMasterDefaultData>({} as IMasterDefaultData);

  const [childrens, setChildrens] = useState<IMasterProgram>({} as IMasterProgram);

  const [lineFirstItem, setLineFirstItem] = useState<IMasterDefaultData>({} as IMasterDefaultData);
  const [lineSecondItem, setLineSecondItem] = useState<IMasterDefaultData>(
    {} as IMasterDefaultData
  );

  const [firstTeachers, setFirstTeachers] = useState<IMasterTeacher[]>([]);
  const [secondTeachers, setSecondTeachers] = useState<IMasterTeacher[]>([]);
  ...
  const getNodeQuestion = useCallback(async () => {
    try {
      ...
      const { data } = await MasterProgramServices.getData(state.childrenId);
  ...
  const getTeachersData = useCallback(async () => {
    try {
      ...
      const teachersFirstList = await MasterProgramServices.indexTeachers(
        lineFirstItem?.childrens[0]
      );

      setFirstTeachers(teachersFirstList?.data);

      const teachersSecondList = await MasterProgramServices.indexTeachers(
  ...
  const handleDeleteTeacher = async (teacher: IMasterTeacher, question: IMasterDefaultData) => {
    try {
      setLoading(true);

      const { data } = await MasterProgramServices.deleteTeacher({ teacher, question });
  ...
  const handleCreateTeacher = async (data: { question: IMasterDefaultData }) => {
